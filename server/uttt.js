...
export function join(ws, game_id, automatic) {
    ...
    if (!game) {
        if (automatic) return client.clearBoard(ws, game_id)
    ...
    client.updateState(ws, game_id, game.board_depth, game.board_state, game.active_layer, game.active_grid_num, game.getClientPiece(ws), game.active_player === 0 ? 'cross' : 'nought', game.moves, game.start_time, game.end_time, game.endless);
}
...
export class Game {
    constructor(game_id, size) {
        ...
        this.active_grid_num = undefined;
        ...
        this.active_layer = this.board_depth;
        this.active_grid_num = 0;
    }
    ...
    place(ws, cell_layer, cell_number, previous_cells) {
        ...
        if (cell_layer === 0) {
            ...
            this.subscribers.forEach(subscriber => {
                client.pieceUpdate(subscriber, this.game_id, this.queued_pieces, this.active_layer, this.active_grid_num, this.active_player === 0 ? 'cross' : 'nought', this.moves);
            });
            this.queued_pieces = [];
        }
    ...
    findNextActiveGrid(grid_layer, grid_number, pos_in_grid, previous_cells) {
        ...
        this.active_grid_num = next_number
    ...
    isCellActive(cell_layer, cell_number) {
        ...
        if (this.active_layer == cell_layer && this.active_grid_num == cell_number)
            return true;
    ...
    expand() {
        ...
        this.active_layer = new_board_depth;
        this.active_grid_num = 0;
        this.subscribers.forEach(ws => {
            client.updateState(ws, this.game_id, this.board_depth, this.board_state, this.active_layer, this.active_grid_num, this.getClientPiece(ws), this.active_player === 0 ? 'cross' : 'nought', this.moves, this.start_time, this.end_time, this.endless);
        });
    }
}
